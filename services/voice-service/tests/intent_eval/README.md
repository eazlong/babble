...
| `cases.jsonl` | 可判分用例：`closed_set` 47 条 + `negative` 34 条 + `person_name` 10 条（合计 91 条） |
...
- `provenance`：`agent_authored` / `human_handwritten` / `real_log`。
  **当前 91 条全部是 `agent_authored`**（由编码 agent 依客户端真实上下文形状编写，**不是**真实玩家语音，也不是运行期 LLM 采样）。这意味着分布偏差是真实存在的：儿童真实表达、口音、ASR 误听模式都未覆盖。计划文档 §8 要求的"20–30 条真实样本锚点"仍是 TODO。（当前待裁定 4 条）
...
- **`rule_006`（原 `pr_004`）**：填槽同时附带提问/寒暄仍算 `provide`，但 `extracted` 必须是**裸名**（去壳 + 丢弃附带内容）。理由是客户端的 `_extract_name` 只剥五个固定前缀，整句会让『，你呢？』留在**被 `save_progress()` 持久化的玩家名字**里；前缀表里没有『叫我』，所以那类表达必须由 voice-service 直接给裸名。锚点 `cs_051`/`cs_054`-`cs_057`。
