...
| `cases.jsonl` | 可判分用例：`closed_set` 47 条 + `negative` 34 条 + `person_name` 3 条（合计 84 条） |
...
- `provenance`：`agent_authored` / `human_handwritten` / `real_log`。
  **当前 84 条全部是 `agent_authored`**（由编码 agent 依客户端真实上下文形状编写，**不是**真实玩家语音，也不是运行期 LLM 采样）。这意味着分布偏差是真实存在的：儿童真实表达、口音、ASR 误听模式都未覆盖。计划文档 §8 要求的"20–30 条真实样本锚点"仍是 TODO。（当前待裁定 6 条）
