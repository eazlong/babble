...
- 后端 auth-service：7 个 API 端点（dashboard、reports、time-limit、content-settings、delete-data）
- 数据库迁移 018：scene_access JSONB + is_active 列
- 前后端 API 对齐：login 返回 token+parent_id，register 支持 child_name 并返回 token
