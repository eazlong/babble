...
### 6. 多候选仲裁（`rule_002`，四级全函数）

**本轮目标优先**（候选出现在 `npc_question` 里 → 取之）→ **最长**（子串关系取更具体者）→ **首次出现位置** → **候选表顺序**。`extracted[key]` 必须**恰好等于某个候选**：禁止拼接、禁止整句。
...
- **定义完成**：以下验收目标转绿，且不新增退步。按 2026-09-16 的 N=5 实测，分三类（分类本身是回归的判据，别只看总数）：
  - **确定性失败 8 项**（`applied_pass_rate = 0.00`，规则层必须修好）：`cs_009`（取候选规范值）、`cs_041`（本轮目标优先）、`cs_044`/`cs_045`/`cs_061`（学习错误被改写）、`cs_047`（封闭题不该出 delegate）、`cs_065`/`cs_066`（迟疑句被当确认）。
  ...
  → 合计 **12 项**均须转绿；`baseline.json` 的 `note` 记录同一清单。
