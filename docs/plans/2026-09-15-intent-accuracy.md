...
## 16. 规则层落地（方向 C 第一段）：纯新文件 + 回放验证

> 出于协作边界（`src/services/asr_postprocess.py` 有并发写入者），本段**不接入生产路径**，只新增文件 + 用已存储的真实模型输出做回放验证。

### 16.1 交付物

| 文件 | 说明 |
|---|---|
| `services/voice-service/src/services/intent_rules.py` | 规则层本体：纯函数、无状态、不调 LLM。**新文件，零冲突** |
| `services/voice-service/tests/test_intent_rules.py` | 34 条单测，把 `rule_001`–`rule_011` 逐条变成可执行断言（含两个"绝不"） |
| `services/voice-service/scripts/replay_intent_rules.py` | 回放验证器：读评测报告的逐例真实模型判决 → 跑规则层 → 报靶子变化；两种模式并列 |

### 16.2 关键设计（都由实测数据决定，不是拍脑袋）

**召回边界 = 词数守恒 + 字符距离 ≤ 20%**。为什么必须两个条件：

| 用例 | 归一化编辑距离 | 词数 | 结局 |
|---|---|---|---|
| `cs_019` Nice to **meat** you | 1（7.7%） | 4 = 4 | 纠正（识别误差） |
| `cs_042` Nice to **eat** you | **2（15.4%）** | 4 = 4 | 纠正（识别误差） |
| `cs_043` Nice to **meetchu** | **2（15.4%）** | 3 ≠ 4 | 不纠正（决议：排除） |
| `cs_044` Nice **meet** you | **2（15.4%）** | 3 ≠ 4 | 不纠正（学习错误：漏 to） |

`cs_042`（必须纠错）与 `cs_044`（不得纠错）的**编辑距离完全相同**，阈值调不动；唯一能分开它们的是**词数**。所以候选对齐跨度必须词数相等，跨度内才允许字符级差异。

**精确命中优先于模糊命中**。字符距离同样无法区分"eat→meet（近音误听）"与"my→too（凭空补词）"：`cs_058`『Nice to meet you, my name is Tom』的 5 词窗口 `[nice,to,meet,you,my]` 与候选 `Nice to meet you too` 只差 2 字符，规则层会凭空补出玩家没说过的 `too`。加"有精确命中则只取精确命中"后解决。

**两个运行模式**（因为 `rule_003` 的权限边界与部分裁定的落地方式存在张力）：
- `strict`：严格 rule_003 —— 规则层**完全不碰意图**。
- `vetoes`：额外允许两处**窄口径合法性归一** —— ① `rule_005`：未声明 `delegatable` 的槽位上把 `delegate` 归一为 `off_topic`；② `rule_010`：命中候选之后紧跟撤回标记时把 `provide` 归一为 `off_topic`。

### 16.3 回放结果（101 例，真实模型输出）

| 模式 | 模型原判 | 规则层处理后 | 修好 | **弄坏** |
|---|---|---|---|---|
| `strict` | 90/101 | **98/101** | 8 | **0** |
| `vetoes` | 90/101 | **101/101** | 11 | **0** |

- `strict` 修好 8 项，`vetoes` 修好 11 项；差异正是 `cs_047`（delegate 门控）、`cs_065`/`cs_066`（撤回检测）——这三项**只能**靠那两处合法性归一，因为模型自己判不出（实测）。
- 两种模式**零弄坏** —— 满足棘轮纪律（新逻辑不得让任何既有通过用例退步）。
- 未接入生产路径：`vetoes` 模式让规则层在两处改了 `intent`，这与 ADR-0009「规则层不覆盖意图」的字面表述存在张力。**需显式裁定**：要么把这两处"合法性归一"写进契约（它们不是意图推断，而是"该标签在当前上下文没有消费者"的修正），要么改由 `_system_prompt` 措辞承担（需改 `src/`）。

### 16.4 三处勘误（实现期发现，均是我的记录错误）

1. **`rule_004`：`cs_042` 是 2 处编辑，不是 1 处**（`eat`→`meet` 需插 `m` 再换 `a→e`）。这使初稿的 15% 阈值与自己的锚点自相矛盾（2/13 = 15.4% > 15%，`cs_042` 会被拒）→ 阈值改为 **0.20**。
2. **`rule_002`：`mynameislily` 是 12 字不是 13 字**。因此 `cs_040` 并非"等长并列"，而是要靠"第 2 级只对子串关系生效"这条细化才落到"首次出现"。原改判结论不变。
3. **`rule_002` 需要三条细化**，每条都由一个**被弄坏的既有用例**逼出来（子串关系排在目标之前、引号目标强于包含关系、精确优先）—— 见 ADR-0009 §6 的对照表。

### 16.5 校准决议与基线

- **`cs_043` 决议为"排除"**（词数 3≠4，按"候选内部不得增删词"拒绝纠正），其期望值随之改为 `provide` + 空 `extracted`。
- 该例原为通过 → 现为未过，属**期望值变更**（非新增用例、非代码改动），基线按逐例数据**精确重算**：`closed_set` 0.830 → **0.811**，总体 0.9069 → 0.897。
- **验收目标 12 → 13 项**（新增 `cs_043`）。
...
## 18. 规则层接入生产（2026-09-16）：两轮 N=5 实测，从 0.948 到 1.000

§17.6 的剩余项①（规则层接入 `process()`）已完成。这一节记录**实测过程**，因为第一轮跑出来的七个失败用例，每个都指向一个真问题。

### 18.1 落地内容

| 项 | 落点 |
|---|---|
| 调用规则层 | `ASRPostprocessor.process()`：模型判决之后、构造响应之前 |
| 五值标签 | `IntentLabel` 3 → 5（`accept`/`reject`），`_system_prompt` 同步 |
| 判据字段 | 响应新增 `verdict_source` / `matched_rule` / `matched_candidate`（`shortcut_*` 暂不落地，见 ADR §3 的理由） |
| 合法性归一 | 两处 → **三处**（新增 `rule_014`：无提议态时 `accept`/`reject` → `off_topic`） |
| 提示词 | delegate 门控、accept/reject 的提议态前置、复读检测、纯语气词 |

### 18.2 第一轮 N=5（104 例）：0.948，门禁失败

closed_set 0.921（+11.0pt）、negative 0.971（**−2.3pt，退步**）、总体 0.948、抖动率 0.010。7 个失败用例，四个根因：

| 根因 | 命中用例 | 性质 |
|---|---|---|
| **决策跑在 `corrected_text` 上** | cs_043/cs_044/cs_045/cs_061 | **接线错误（我引入）**：模型已把学习错误纠成正确答案，规则层在纠错后的文本上"确认命中"，等于把孩子的错误改写成满分答案——正是 rule_004 立规矩要防的事 |
| `reject` 无提议态门控 | cs_046 | 扩五值后暴露的新标签缺口（`normalize_delegate` 只覆盖 `delegate`） |
| 提示词改动引起的判性漂移 | neg_017、neg_003 | 这两例在 9/15 的**四次历史运行里都是 `off_topic`**，改后变 `provide`；不是既有噪声 |
| 模型采样波动 | neg_003（同轮内） | 需要与上一条区分：同轮 5 次里既有 off_topic 也有 provide |

**关键区分动作**：翻 `reports/` 里的历史报告逐例取证，才把"我改坏的"与"本来就抖的"分开——只看当轮聚合数字是分不出来的。

### 18.3 修法与第二轮 N=5：全桶 1.000

三项修复（详见 ADR §11 与 `rule_012`/`rule_013`/`rule_014`）：

1. **规则层输入拆成两个文本**：决策用**原始 ASR 文本**，自由槽表面形式用 `corrected_text`。一般化的教训写进了 ADR §11——*模型与规则层串联时，规则层必须看到模型的**输入**，而不只是它的输出*。
2. **`accept`/`reject` 的合法性归一**（`rule_014`，走 ADR 修订入例外表）。
3. **提示词补两条被裁定锚定的界限**：复读 `recent_turns` 里的旧答案 → off_topic；纯语气词 → off_topic、绝不猜值。

第二轮结果：

| 桶 | 样本 | 准确率 | 与基线 |
|---|---|---|---|
| closed_set | 56 | **1.000** | +18.9pt |
| negative | 34 | **1.000** | +0.6pt |
| open_slot | 3 | **1.000** | +0.0pt |
| person_name | 11 | **1.000** | +0.0pt |
| **总体** | 104 | **1.000** | — |

抖动率 0.000（N=5、520 次测量零翻转）、降级率 0.0096（5/520，网关 500 所致）、门禁**通过**。13 项验收目标逐例 `pass_rate=1.0`。

### 18.4 顺带发现并修掉的两个规则层缺陷

都不是新功能，而是"本层不该做的事"。**两条都是接入生产时才暴露的**——单测与离线回放都跑在存量模型输出上，接不起来看不见（详见 ADR §10）。

- `rule_012`：`CHAR_EDIT_MIN=1` 对**单字候选**等于"任何字都算命中"（`find_hits("我觉得是B", ["A"])` → 5 个命中）。字母识别是生产上真实的候选形态。
- `rule_013`：自由槽里模型给出**更紧**的值（「小北」⊂ 本层重算的「小北你是谁」）时，本层不得覆盖它。

用例集同时补了 3 例（cs_068/cs_069/cs_070）——原有 101 例**没有任何长度 < 3 的候选**，`rule_012` 的问题在用例层面根本不可见。

### 18.5 一个必须记住的测量口径变化

规则层接入后，**端到端准确率量的是"模型 + 规则层"这个系统**，不再是模型单独的水平：实测"一律 delegate"的退化 stub 从 <0.1 涨到 **0.55**，因为 `rule_005` 的归一恰好把它的退化策略在 negative 桶上救成了正确结论。

连带影响：
- `test_degraded_stub_fails_gate_naming_the_bucket` 的阈值从 `<0.1` 改为"相对满基线大幅下降"，且不再断言 negative 桶退步（它现在反而变对）——门禁看**逐桶**的价值没有变，但"退化模型必然在所有桶上崩"这个直觉不再成立。
- 要量**模型本身**，必须关掉规则层（`RuleOptions(intent_vetoes=False)` 或回放器的 `strict` 模式）。

### 18.6 遗留（按优先级）

1. **拼音/近音维未实现**（ADR §9 已更正这一事实）：`RuleOptions.phonetic` 只是配置项，无实现、无 `pypinyin` 依赖。可见后果是模型把「暑假」按同音纠成候选「书架」时规则层弃权（行为安全——`corrected_text` 透出、客户端回退，玩家看到的仍是对的，只是少了一层背书）。边界用例 `test_postprocessor_abstains_on_homophone_it_cannot_verify`。**建议**：补 `pypinyin` + 同音匹配（同词数 + 逐词无声调拼音相等），并补 3–5 例同音用例。
2. **`rule_010` 的客户端门控在规则层接入后才真正生效**：`_is_retraction_downgrade` 现在能拿到 `matched_rule` 了（此前 `matched_rule` 恒为空、门控空转）。建议做一次真人验收：说「出发，等一下」应被重问，而不是切场景。
3. **回放证据缺口**：`tests/intent_eval/reports/` 被 gitignore，§16.3 所依据的那批报告**已不在盘上**（只剩 9/15 16:19–16:24 的四份、覆盖 70 例）。本次两轮报告已补上 104 例的逐例数据；建议把报告目录改为保留最近 N 份的滚动快照，避免"结论在、证据没了"。
4. `shortcut_hit`/`shortcut_confidence` 仍未落地（无 LLM 前的短路路径，填了就是假判据）。

### 18.7 成本

两轮全量 N=5（各 104 例）共约 1035 次 LLM 调用（含 147 次有界重试），单轮总耗时约 5 分钟。基线已按"被测代码已变 → 标定验证后合并"的规则更新为本次结果（独立提交）。
