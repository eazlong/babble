...
2. `My name is Lily, nice to meet you` → 返回 `mynameislilynicetomeetyou` —— 一个**既不是候选、也不是原话片段**的拼接值。
...
**落地**：新增 4 条可判分用例 `cs_037`–`cs_040` 分别钉住第 1、1、2、3 级；`pr_010` 移出待裁定集。**下游新问题 `pr_011`**：模型返回非候选值（拼接）时 post-processor 该丢弃 extracted、就地改写、还是判 `off_topic`？rule_001/rule_002 只规定了"应该取候选"，没规定"取不到时怎么办"——方向 C 落地前必须有人回答。
