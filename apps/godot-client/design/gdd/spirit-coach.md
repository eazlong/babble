...
- 新增 §3.6 Spark 示范模式（对齐 game-concept.md/core-loop.md/lxp-system.md: ASR 连续失败 2 次）
- 澄清示范模式与 DDR 的两层关系（2 次 vs 3+ 次触发阈值）
- Player Fantasy 添加魔法能量叙事框架（对齐 game-concept.md）
...
**Spark 角色定义**：Spark 是玩家在游戏世界中的**随身精灵伙伴**，既是故事中的 NPC 角色（陪伴、对话、任务引导），也是后台 AI Agent 的视觉化身（异步监控、干预提示）。Spark 具有**双模式架构**：

- **NPC 模式**：玩家主动与 Spark 对话，完成教学引导、任务介绍、情感互动
- **Agent 模式**：系统异步检测玩家状态（错误/沉默/求助），Spark 主动浮现提供干预提示

**系统定位**：Spirit Coach 是旁路辅助系统，不阻塞主对话流程。当玩家出现学习困难时，Spark 以非侵入方式浮现，提供鼓励式提示，然后自动淡出，将焦点还给 NPC 对话。

---

## 2. Player Fantasy

**玩家体验目标**："Spark 是我的魔法伙伴，我遇到困难时它总会出现帮我"

**叙事包装（与 game-concept.md "魔法能量"框架对齐）：**
Spark 是玩家魔法旅程中的**同伴精灵**——它不评判玩家的魔法能量强弱，而是在玩家需要时温柔出现，帮助激活更强的魔法。Spark 的提示和鼓励是"魔法伙伴的支持"，而非"系统的纠正"。

**情感设计（MDA 美学）**：
- **Fellowship（陪伴感）**：Spark 像朋友一样存在，不是冷冰冰的提示框
- **Expression（表达支持）**：Spark 的提示鼓励而非评判，让玩家敢开口
- **Submission（安心感）**：知道 Spark 会在困难时出现，降低尝试焦虑

**角色一致性保证**：
- 无论 NPC 模式还是 Agent 模式，Spark 的**视觉形象一致**（同一只精灵）
- **性格一致**：温柔、鼓励、耐心，从不批评
...
│   (主动对话)         │           (异步干预)                        │
├─────────────────────┼───────────────────────────────────────────┤
│ • 玩家点击 Spark     │ • 系统监控触发 (Redis Streams)            │
│ • Spark 主动搭话     │ • 错误检测 → 冷却判断 → 干预生成           │
...
**模式切换规则**：
- NPC 模式优先级高于 Agent 模式（如果玩家正在与 Spark 对话，不触发干预）
- Agent 模式触发时，如果 Spark 已在屏幕中（NPC 模式），直接显示气泡提示
...
| `enter` | STATE_ENTER | 5 | 飞入动画，透明度+缩放 | Spark 进入屏幕 |
| `exit` | STATE_EXIT | 6 | 淡出动画，缩小消失 | Spark 离开屏幕 |
...
- 当 `struggle_score >= 0.67` 时，同时触发：
  - Spark Agent 模式介入（显示鼓励提示）
  - Quest 系统难度降级（降低任务类型）
  - Star Economy 保底机制（最低 2 星）
- 确保玩家体验到统一的支持，而非分裂的标准

### 3.6 Spark 示范模式（ASR Fallback）
...
| **第二层：DDR（动态难度调整）** | `struggle_score >= 0.67`（连续错误 **>= 3 次** + 连续低 LXP + ASR 高失败率） | Spirit Coach + Quest + Star Economy | Spark 鼓励提示 + 任务降级 + 保底 2 星 |
...
**触发阈值**：
- $\text{struggle\_score} \geq 0.67$ → Spark 介入 + Quest DDR + Star 保底
- 三个系统同步响应，避免分裂体验
...
- 重连成功显示 "Spark 回来了" 的轻量提示

### 5.4 Spark 已在屏幕中

**场景**：玩家正在与 Spark（NPC 模式）对话时触发 Agent 干预

**处理规则**：
- NPC 模式优先级高于 Agent 模式
- 延迟干预，直到 NPC 对话结束
- 或：在 Spark 气泡上方叠加一个小提示图标（非侵入式）
...
- [ ] 玩家说 "help" → 3 秒内 Spark 显示 "我来帮你！你可以说：Can you help me?"
- [ ] 玩家沉默 10 秒 → Spark 显示鼓励提示，30 秒内不重复触发（与 core-loop.md 对齐）
- [ ] 玩家说 "I am go" → Spark 显示 "差一点点！可以说：I am going"
- [ ] 同一错误在 20 秒内重复 → 只触发一次干预
- [ ] NPC 模式下不触发 Agent 干预
- [ ] ASR 连续失败 2 次 → Spark 示范模式触发：NPC 播放正确发音，玩家跟读
...
- [ ] 6-10 岁测试玩家能正确理解 Spark 的提示意图
- [ ] 测试玩家不感到 Spark 干扰正常游戏
- [ ] Spark 的鼓励语气符合儿童心理预期
...
> Spark 是 LinguaQuest RPG 中的**统一精灵角色**，既作为玩家主动对话的 NPC 伙伴，也是系统异步监控的 AI Agent 视觉化身。
>
> 两种模式（NPC 模式与 Agent 模式）使用**相同的视觉形象、性格设定和交互风格**，确保儿童玩家的认知一致性。
>
> "Spirit Coach" 是系统层面的功能名称，指代后端 AI 监控与干预能力，其视觉呈现始终是 Spark 角色。

**文档历史澄清**：
- SRS 中的 "Spirit Coach Agent" → 指代后端系统的异步监控能力
- 剧情设计中的 "Spark" → 指代统一的精灵角色
- 无角色分裂，无第二精灵实体
