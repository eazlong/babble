...
**负责服务**: quest-service
**依赖系统**: content-service, dialogue-service, assessment-service, reward-service, spirit-coach-service, voice-service（上游 ASR）, star-economy（下游星星映射）, lxp-system（下游 LXP 累积）, parent-dashboard（下游进度报告）, Godot 客户端（下游 UI）
...
**玩家应该感觉**: "每天都有新挑战，完成任务就能获得星星和奖励"

**情感设计**：

| 情感目标 | 设计实现 | 具体表现 |
|---------|---------|---------|
| **目标感** | 清晰的任务列表和目标 | 屏幕侧边显示当前任务，Spark 提示"下一个任务：和 TreeSpirit 打招呼" |
| **成就感** | 任务完成的仪式反馈 | 任务完成时播放音效+粒子特效，显示"Task Completed!" |
| **期待感** | 日常任务的每日刷新 | 每日首次进入游戏时 Spark 展示"今日任务"，带神秘感 |
| **安全感** | 任务失败不惩罚 | 任务可无限重试，Spark 在困难时提供选项提示 |
...
|------|-----------|----------|-------------|
| SpiritForest | 3 | Greeting → Color → Nature | Hello, Hi, Red, Blue, Tree, Flower |
| SpellLibrary | 4 | Classroom → Spell → Request → Magic | Stand up, Sit down, Please, Can I... |
| RainbowGarden | 4 | Number → Count → Describe → Create | One to Twenty, How many..., Big/Small |
...
| 转换 | 触发条件 | Spark 行为 |
|------|---------|-----------|
| LOCKED → AVAILABLE | 前置任务完成 | Spark 飞出提示"新任务解锁！" |
| AVAILABLE → IN_PROGRESS | 玩家接近任务 NPC | NPC 主动对话，任务开始 |
| IN_PROGRESS → COMPLETED | 评估达到 3 星 | 播放完成动画，发放星星 |
| IN_PROGRESS → FAILED | 连续 3 次 1-2 星 | Spark 介入，提供选项辅助 |
...
| ID | 任务名称 | 类型 | 描述 | 预期星星 |
|----|---------|------|------|---------|
| DQ01 | 晨间问候 | 口语重复 | 向 Spark 说早安问候语 | 1 |
| DQ02 | 听音辨词 | 听力理解 | 听 NPC 发音，选择正确图片 | 1 |
| DQ03 | 描述花园 | 图片描述 | 描述 RainbowGarden 中的植物颜色 | 2 |
| DQ04 | 自由聊天 | 自由对话 | 和 Spark 自由对话 3 轮 | 2-3 |
...
| 规则 | 说明 |
|------|------|
| 已完成主线任务可重玩 | 玩家可重复进入已完成的场景，重新执行任务获取星星和 LXP |
| 剧情推进不重复 | 仅首次完成推进场景剧情和解锁下一场景 |
| 场景 Badge 不重复解锁 | 每个场景 Badge 仅解锁一次 |
| 每日重置 | 每日首次进入场景时，场景星星计数器清零（与 star-economy §3.3 对齐） |
| 日常任务独立 | 日常任务不受主线重玩影响，按每日刷新规则独立运行 |

**设计意图**：允许高水平玩家通过重玩提升表现、积攒星星用于商店消费，与 star-economy 的"场景可重复刷星"机制一致。
...
IF avgLXP < 40:
    - 下任务难度降一级（最低 Level 1）
    - Spark 提示"让我帮你一起完成这个简单的任务"
IF 当前场景连续 3 次任务评估 avgLXP >= 80:
    - 下任务难度升一级（最高 Level 4）
    - Spark 提示"你做得真棒！试试更难的任务吧"
```

完整公式见 §4.4 `shouldTriggerBasicDDR` / `calculateDDRAdjustment`。

**干预级 DDR**（跨系统安全网，触发 Spark 介入 + 保底机制）：

建立跨系统共享的 `player_struggle_score`（与 spirit-coach.md §3.5、star-economy.md §5.2 对齐），确保 Spark 介入和任务降级同步触发：
...
if struggle_score >= 0.67:
  trigger_spark_intervention()           // Spark 介入
  activate_quest_ddr()                   // Quest 系统降级
  // Star Economy 启动保底机制（最低 2 星）— 由 star-economy 响应
```

**两层关系**：基本 DDR 是常规调节（每任务后），干预级 DDR 是安全网（多维信号同时恶化时触发）。两者可同时生效：基本 DDR 降级难度，干预级 DDR 同时触发 Spark 辅助和保底。
...
interface QuestProgress {
    ...
    starsEarned: number;            // 已获得星星
    targetStars: number;            // 目标星星数（来自 star-economy 动态阈值 calculate_badge_threshold）
...
**任务类型与典型星星范围**（基于 star-economy.md §4.1 LXP→星星映射，按任务复杂度估算典型区间）：

| 任务类型 | 典型星星范围 | 任务数量/场景 | 说明 |
|---------|-------------|--------------|------|
...
> **注意**：实际星星数由 assessment-service 按 LXP 公式计算后经 star-economy 映射得到，上表仅为场景平衡设计参考。
...
IF 今日 3 个日常任务全部完成:
    ...
    - Spark 祝贺动画: "太棒了！你完成了今天的所有任务！"
```

### 3.5 任务失败处理（Spark 介入、保底机制）

**无失败惩罚原则**：
- 任务永远不会"失败"导致进度丢失
- 低表现触发 Spark 介入辅助，而非惩罚

**Spark 介入矩阵**：

| 场景 | 检测条件 | Spark 行为 | 保底效果 |
|------|---------|-----------|---------|
| 连续低星 | 当前任务 3 次尝试 < 40 LXP | Spark 提供选项提示（A/B/C 选择） | 下次最低获得 2 星 |
| 长期停滞 | 单任务时长 > 5 分钟 | Spark 建议"休息一会儿再继续" | 自动保存进度 |
| ASR 困难 | 连续 3 次 ASR 置信度 < 0.3 | Spark 降低任务难度，改为单词级 | 显示单词卡片辅助 |
| 完全卡住 | 玩家说"我不会"或静默 > 15s | Spark 播放示范音频，允许跟读 | 跟读即算 2 星完成 |
...
| **第三层：场景级保底解锁（安全网）** | 场景内总回复数 > 25 次 AND 当前场景星星 < threshold | 后续所有回复最低 2 星，Spark 提示"你已经很努力啦，让我帮你一起完成！" | 本段下方 |

**场景级保底解锁详细规则**：

```
IF 场景内总回复数 > 25 次 AND 当前星星 < threshold:
    - 触发"保底解锁"模式
    - 剩余所需星星自动按 2 星/回复计算
    - Spark 提示"你已经很努力啦，让我帮你一起完成！"
    - 所有后续回复最低获得 2 星
```
...
// Spark 介入事件（发送到 spirit-coach-service）
interface SparkInterventionEvent {
...
function calculateQuestProgress(
    currentStars: number,
    targetStars: number    // 来自 star-economy 动态阈值 calculate_badge_threshold(scene_id, player_avg_stars)
): number {
    return Math.min(100, Math.round((currentStars / targetStars) * 100));
}
```

**进度映射**（以 SpiritForest 中等玩家 threshold=15 为例）：
| 星星占比 | 进度百分比 | Spark 提示语 |
|--------|-----------|-------------|
| 0-25% | 0-25% | "刚开始，加油！" |
| 25-50% | 25-50% | "已经四分之一啦！" |
| 50-75% | 50-75% | "过半啦，继续！" |
...
**场景预期时间计算**：
```typescript
// SpiritForest 场景（3 个任务：口语重复 + 听力理解 + 图片描述）
...
| 星星数异常 | 单次获得 > 5 星 | 拒绝并回滚，通知开发团队 |
| 负星星数 | 计算结果为负数 | 归零处理，记录错误日志 |
...
| spirit-coach-service | 介入建议、提示语 | 困难时的 Spark 介入 |

### 6.2 下游依赖（输出去向）

| 系统 | 数据 | 用途 |
|------|------|------|
| reward-service | 任务完成事件、星星数 | 奖励计算和发放 |
| star-economy | 星星累积、阈值状态 | 场景完成判断 |
...
| `FLOOR_STARS_MIN` | `assets/data/quest_config.json` | 2 | 1-3 | Gate | 保底机制最低星星数（三层保底共用，§3.5） |
...
| `SCENE_STAR_THRESHOLD` | `assets/data/quest_config.json` | 动态(15-25) | 10-30 | Curve | 场景 Badge 解锁动态阈值 |
| `MAX_ATTEMPTS_PER_QUEST` | `assets/data/quest_config.json` | -1 | -1,3-10 | Gate | 单任务最大尝试次数（-1=无限） |

### 7.2 任务类型难度配置

| 任务类型 | 基础难度 | LXP 权重 | 典型星星范围 | 可调范围 |
|---------|---------|---------|-------------|---------|
...
2. SCENE_STAR_THRESHOLD: 20 → 18（更容易解锁 Badge）
...
- [ ] 干预级 DDR 在 struggle_score >= 0.67 时正确触发 Spark 介入 + 保底（§3.2）
- [ ] 场景级保底在场景内总回复数 > 25 次 AND 当前星星 < threshold 时正确激活（§3.5）
...
- [ ] 场景内总回复数 > 25 次且星星不足 threshold 时触发场景级保底解锁（§3.5）
...
- [ ] Spark 介入事件正确发送到 spirit-coach-service
- [ ] Godot 客户端正确显示任务列表和进度
- [ ] 场景完成时正确触发 Badge 解锁事件

### 8.5 体验测试（儿童用户）

- [ ] 6-10 岁测试玩家理解"主线任务"和"日常任务"的区别
- [ ] 测试玩家在任务困难时接受 Spark 的辅助提示
...
```
测试场景: SpiritForest 场景完整流程（验证星星计算链路一致性）
输入:
  - 玩家: 四年级水平
  - 任务: 3 个主线任务（repeat + listen + describe）
  - 表现: 平均 2.5 星/回复（由 star-economy §4.1 LXP→星星映射得出）
  - 预估回复数: 6-8 次（含重试，基于 §4.5 estimateQuestTime）

预期输出:
  - 完成时间: 8-12 分钟
  - 获得星星: 15-20 星（2.5 星/回复 × 6-8 回复）
  - 获得 LXP: 150-250
  - 获得金币: 75-100
  - Badge 解锁: 是（如果星星 >= 动态阈值，参考 star-economy §4.1 `calculate_badge_threshold`）
```
...
**任务流程**：
```
1. Spark 发起对话: "What's your favorite color?"
2. 玩家自由回答（无固定答案）
3. LLM 评估回答相关性和语法
4. 多轮对话（3-5 轮）
5. 基于整体对话质量评分
...
|------|------|------------------|--------|
| star-economy.md | 动态阈值公式 calculate_badge_threshold() | SCENE_STAR_THRESHOLD = 动态(15-25) | ✓ 一致（使用 star-economy 动态阈值，非硬编码） |
...
| core-loop.md | 动态阈值解锁 Badge | SCENE_STAR_THRESHOLD = 动态(15-25) | ✓ 一致（动态阈值机制） |
