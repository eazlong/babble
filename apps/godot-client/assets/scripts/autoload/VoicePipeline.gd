...
var listening_started_at: float = 0.0
...
func start_listening(recording_context: Dictionary = {}) -> void:
	if is_listening:
		return
	# ASR 测试模式：跳过麦克风与静音等待，直接用录制好的音频文件触发 voice_ended。
	if HybridAPI.asr_default_answer_test_enabled:
		_start_test_audio_flow(recording_context)
		return
	_start_microphone()

	is_listening = true
	listening_started_at = Time.get_ticks_msec() / 1000.0
	audio_buffer.clear()
...
func _start_test_audio_flow(recording_context: Dictionary) -> void:
	# 测试模式不打开麦克风，但仍维护 listening 状态，避免场景重复触发。
	is_listening = true
	current_recording_context = recording_context.duplicate(true)
	current_recording_envelope = {}
	listening_started.emit()
	print("[VoicePipeline] test-audio mode: skipping microphone, loading recorded file")
	call_deferred("_emit_test_voice_ended")

func _emit_test_voice_ended() -> void:
	if not is_listening:
		return
	var audio_data := HybridAPI.get_next_asr_test_audio_data()
	if audio_data.is_empty():
		push_warning("[VoicePipeline] test-audio mode: no audio bytes available")
	voice_ended.emit(audio_data)
	if is_listening:
		is_listening = false
		listening_stopped.emit()
...
func _process(delta: float) -> void:
	...
	if current_time - listening_started_at >= _max_recording_duration():
...
func _prepare_recording_envelope(recording_context: Dictionary) -> Dictionary:
	...
	if magic_echo_manager == null or not magic_echo_manager.has_method("prepare_recording_attempt"):
		return {}
	var context := _recording_context_with_session_and_prompt(recording_context, magic_echo_manager)
...
func _recording_context_with_session_and_prompt(recording_context: Dictionary, magic_echo_manager: Node) -> Dictionary:
	var context := recording_context.duplicate(true)
	if str(context.get("child_id", "")).is_empty():
		context["child_id"] = _current_child_id()
	if str(context.get("scene_id", "")).is_empty():
		context["scene_id"] = _current_scene_id()
	...
	if str(context.get("game_session_id", "")).is_empty() and magic_echo_manager.has_method("enter_learning_scene"):
		var session: Variant = magic_echo_manager.call("enter_learning_scene", context.get("scene_id", ""), context.get("child_id", ""))
		if session is Dictionary:
			context["game_session_id"] = session.get("game_session_id", "")
	if str(context.get("prompt_turn_id", "")).is_empty() and magic_echo_manager.has_method("create_prompt_turn"):
		var prompt: Variant = magic_echo_manager.call("create_prompt_turn", _prompt_turn_data_from_context(context))
		if prompt is Dictionary:
			context["prompt_turn_id"] = prompt.get("prompt_turn_id", "")
	return context

func _prompt_turn_data_from_context(context: Dictionary) -> Dictionary:
	return {
		"child_id": str(context.get("child_id", _current_child_id())),
		"game_session_id": str(context.get("game_session_id", "")),
		"scene_id": str(context.get("scene_id", _current_scene_id())),
		"quest_id": str(context.get("quest_id", "voice_attempt")),
		"content_id": str(context.get("content_id", "voice_prompt")),
		"content_version": int(context.get("content_version", 1)),
		"prompt_text_snapshot": str(context.get("prompt_text_snapshot", "Voice prompt")),
		"target_utterance_snapshot": str(context.get("target_utterance_snapshot", "")),
		"expected_answer_type": str(context.get("expected_answer_type", context.get("attempt_type", "short_answer"))),
		"assessment_rule_version": str(context.get("assessment_rule_version", "v1")),
	}
...
func _current_scene_id() -> String:
	...
	if game_manager == null:
		return ""
	return str(game_manager.get("current_scene"))

func _max_recording_duration() -> float:
	# 优先读 recording_context.max_duration（数据驱动，CLAUDE.md §5/§12）。
	if current_recording_context.has("max_duration"):
		var ctx_max: float = float(current_recording_context.get("max_duration", 0.0))
		if ctx_max > 0.0:
			return ctx_max
