...
Ask the user directly in conversation for implementation choices and next-step decisions. When an interactive user-input tool is available and the current mode permits it, use it with concise labels. Follow the **Explain -> Capture** pattern: explain options in conversation, then capture the decision. Batch only genuinely independent questions.
