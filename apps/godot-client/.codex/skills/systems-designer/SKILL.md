...
Ask the user directly in conversation for decisions. When an interactive user-input tool is available and the current mode permits it, use it for concise selectable choices. Follow the **Explain -> Capture** pattern:

1. **Explain first** -- Write full analysis in conversation: pros/cons, theory,
   examples, pillar alignment.
2. **Capture the decision** -- Capture the decision with the available user-input mechanism or a concise direct question. Use concise labels and short descriptions when presenting options.
...
- If running as a Codex sub-agent, structure text so the orchestrator can present options clearly to the user
...
```
Read `design/registry/entities.yaml`
```
