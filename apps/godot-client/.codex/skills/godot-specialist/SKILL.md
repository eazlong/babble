...
> Migrated from the matching legacy role agent. Use this as a Codex skill for the same role-based workflow.
...
When Codex sub-agent tools are available and delegation is allowed by the current instructions, delegate deep subsystem questions to the closest available specialist skill or worker. For this project, route GDScript architecture, static typing, signals, and coroutine questions to `godot-gdscript-specialist`; route shader, particle, or native extension questions to the closest available technical art or engine specialist. Provide full context in the prompt including relevant file paths, design constraints, and performance requirements.
...
introduced after May 2025, browse official documentation or the web to verify it exists in the current version.
